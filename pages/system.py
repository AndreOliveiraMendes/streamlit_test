...
from pages.utils.system_util import generate_extended_matrix, generate_auxiliar_matrix, decode_step, scale_by_a_factor
...
def write_system_solution(matrix, num_stats, solution_introduction = None):
    total = len(matrix[0])
    head = [f"pt_{{{i + 1}}}" for i in range(num_stats)] \
        + [f"mt_{{{i + 1},{j + 1}}}" for i, j in combinations(range(num_stats), 2)] \
        + [f"s_{{{i + 1}}}" for i in range(num_stats)]
    latext_code = ""
    if solution_introduction is not None:
        latext_code += f"{solution_introduction}\n\n"
    latext_code += "$$\begin{cases}"
    st.write(matrix)
    return
    for i in range(num_stats):
        equation = []
        for j in range(num_stats, total):
            if matrix[i][j] == "0":
                continue
            if '-' in matrix[i][j]:
                equation.append(f"+{matrix[i][j].replace('-', '')}*{head[j]}")
            else:
                equation.append(f"-{matrix[i][j]}*{head[j]}")
        equation
